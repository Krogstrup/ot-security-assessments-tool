...
pub fn generate_report(
    ...
    let font_family = font_family.ok_or_else(|| {
        ReportError::Pdf(
            "Could not find LiberationSans fonts. Install liberation-sans-fonts package."
                .to_string(),
        )
    })?;
    ...
    doc.set_title(
        config
            .title
            .as_deref()
            .unwrap_or("ICS/SCADA Network Assessment Report"),
    );
...
fn add_title_page(doc: &mut Document, config: &ReportConfig, data: &ReportData) {
    ...
    push_meta_row(
        &mut table,
        "Protocols Detected:",
        &data.protocol_stats.len().to_string(),
    );
    ...
    // Tool credit
    let credit_style = style::Style::new()
        .with_font_size(8)
        .with_color(genpdf::style::Color::Rgb(100, 116, 139));
    doc.push(
...
fn add_executive_summary(doc: &mut Document, data: &ReportData) {
    ...
    let ot_assets = data
        .assets
        .iter()
        .filter(|a| !matches!(a.device_type.as_str(), "it_device" | "unknown"))
        .count();
    ...
    let summary = format!(
        ...
        total_assets, ot_assets, it_assets, total_connections, total_protocols, ot_protocols
    );
...
fn add_asset_inventory(doc: &mut Document, data: &ReportData) {
    ...
    for asset in &data.assets {
        ...
        let protocols = if asset.protocols.len() > 2 {
            format!(
                "{}, +{}",
                asset.protocols[..2].join(", "),
                asset.protocols.len() - 2
            )
        } else {
...
fn add_protocol_analysis(doc: &mut Document, data: &ReportData) {
    ...
    push_header_row(
        &mut table,
        &["Protocol", "Packets", "Bytes", "Connections", "Devices"],
    );
    ...
    for stat in &data.protocol_stats {
        ...
        push_data_row(
            &mut table,
            &[&stat.protocol, &packets, &bytes, &conns, &devices],
        );
    }
...
fn push_meta_row(table: &mut elements::TableLayout, label: &str, value: &str) {
    ...
    row.push_element(elements::Paragraph::new(value).padded(Margins::trbl(2, 4, 2, 4)));
    row.push().ok();
}
...
fn generate_auto_findings(data: &ReportData) -> Vec<(&str, &str, String)> {
    ...
    let unencrypted_ot = data
        .protocol_stats
        .iter()
        .filter(|p| {
            matches!(
                p.protocol.as_str(),
                "modbus" | "dnp3" | "s7comm" | "bacnet" | "profinet"
            )
        })
        .count();
    if unencrypted_ot > 0 {
...
mod tests {
    ...
    fn test_auto_findings() {
        ...
        assert!(findings
            .iter()
            .any(|f| f.1 == "Public IP Addresses Detected"));
    }
