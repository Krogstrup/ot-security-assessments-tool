...
pub(super) fn add_routes(router: Router<SharedState>) -> Router<SharedState> {
    router
        ...
        .route(api_path::DATA_CONNECTION_PACKETS_BY_ID, get(get_connection_packets))
...
async fn get_function_code_stats(State(state): State<SharedState>) -> Result<Json<Value>, ApiError> {
