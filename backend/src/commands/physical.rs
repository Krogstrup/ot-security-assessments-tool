...
pub fn import_cisco_config(path: String, state: &AppState) -> Result<PhysicalTopology, String> {
    ...
    physical.physical_topology.build_links();
    // Re-correlate ARP→port mappings
    physical.physical_topology.correlate_arp_to_ports();

    log::info!(
        "Imported Cisco config for switch '{}' from {}",
        hostname,
        path
    );
...
pub fn import_mac_table(
    ...
    physical
        .physical_topology
        .apply_mac_table(&switch_hostname, &entries);

    // Re-correlate since we have new MAC→port mappings
    physical.physical_topology.correlate_arp_to_ports();

    log::info!(
        "Imported {} MAC table entries for switch '{}'",
        count,
        switch_hostname
    );
...
pub fn import_cdp_neighbors(
    ...
    physical
        .physical_topology
        .apply_cdp_neighbors(&switch_hostname, &neighbors);

    // Rebuild inter-switch links from the updated CDP data
    physical.physical_topology.build_links();

    log::info!(
        "Imported {} CDP neighbors for switch '{}'",
        count,
        switch_hostname
    );
...
pub fn import_network_config(path: String, state: &AppState) -> Result<PhysicalTopology, String> {
    ...
    log::info!(
        "Auto-imported network config for switch '{}' from {}",
        hostname,
        path
    );
    Ok(physical.physical_topology.clone())
...
pub fn import_mac_table_auto(
    ...
    log::info!(
        "Auto-imported {} MAC table entries for switch '{}'",
        count,
        switch_hostname
    );
    Ok(physical.physical_topology.clone())
...
pub fn import_neighbor_table(
    ...
    log::info!(
        "Auto-imported {} neighbor entries for switch '{}'",
        count,
        switch_hostname
    );
    Ok(physical.physical_topology.clone())
