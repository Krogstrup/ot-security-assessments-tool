...
5. Results are serialized using shared models (`gm-models`) and generated TS types (`bindings/gen/types`).
...
- Update Rust model types in `gm-models`, `gm-parsers::models`, or `gm-ingest::models`.
...
### `src-tauri/crates/gm-constants/Cargo.toml`
- For: constants crate manifest.
- Does: minimal crate with shared static domain constants.
- Used in code: imported by parsers, analysis, and processor logic.
...
### `src-tauri/crates/gm-models/Cargo.toml`
- For: shared model crate manifest.
- Does: declares serde/ts-rs related dependencies.
- Used in code: imported by command layer, processors, exports, bindings generation.
- Maintain: avoid app/runtime coupling.

### `src-tauri/crates/gm-models/src/lib.rs`
- For: canonical cross-crate DTOs.
- Does: defines `AssetInfo`, `ConnectionInfo`, `PacketSummary`, `ProtocolStatInfo`, signature match structs.
- Used in code: state storage, command responses, TS generation.
- Maintain: this is a core contract surface; evolve additively and regenerate TS.
